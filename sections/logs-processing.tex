...
Ens han proporcionat aquestes dades que corresponen als accessos desde del primer dia de l'any 2006 fins al darrer dia del 2023.
...
\begin{figure}[htbp]
    ...
    \caption{Exemple del contingut d'un \textit{\gls{log}}.}\label{fig:example-log}
    \source{UPCommons.}
\end{figure}


\noindent
Cada línia present als fitxers de \textit{\gls{log}s} correspon a una petició \gls{HTTP} .
...
\begin{figure}[htbp]
    ...
    \captionsetup{justification=centering}
    \caption{Exemple d'un \textit{\gls{log}} que accedeix a un recurs web, concretament una fulla d'estils \textit{\gls{CSS}} .}\label{fig:log-web-resource}
    \source{UPCommons.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \captionsetup{justification=centering}
\caption{Exemple d'una cadena de \textit{\gls{log}s} que accedeixen al mateix contingut.}\label{fig:log-repetitions}
    \source{\gls{UPCommons}.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \captionsetup{justification=centering}
    \caption{Exemple d'un \textit{\gls{log}} d'una cerca a la plataforma.}\label{fig:log-search}
    \source{UPCommons.}
\end{figure}
...
\noindent
Diem que un \textit{\gls{log}} produeix un error de processament si el seu format difereix abrutment del general (vegeu ~\ref{subsec:log-analysis}).
...
Que es traduiria a:
\begin{center}
    Inserta a la base de dades el \textit{\gls{log}} que compleixi cert criteri i fes-ho amb aquest format.
\end{center}
...
\noindent
Finalment, al component \texttt{Bolcat} arribarà els \textit{\gls{log}s} desitjats amb el format escollit.
Ara bé, depenen de la base de dades on es vol fer l’agregació, és possible que s’hagin de fer petits retocs al \textit{\gls{log}} per compatibilitzar amb la base de dades. \\
